...
pub trait DrawUserInterface {
    fn render(&mut self, ctx: &egui::Context, painter: &mut egui_gl_glfw::Painter);
}
...
impl UserInterface {
    ...
    pub fn render(&mut self, uis: Vec<Box<&mut dyn DrawUserInterface>>) {
        self.begin();
        for ui in uis {
            ui.render(&self.ctx, &mut self.painter);
        }
        self.end();
    }
}
